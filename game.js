...
paperAtlas.decoding = "async";
paperAtlas.src = "assets/paper-atlas.svg?v=20260605-ink-1";
const paperKnight = new Image();
paperKnight.decoding = "async";
paperKnight.crossOrigin = "anonymous";
paperKnight.src =
  location.hostname === "localhost" || location.hostname === "127.0.0.1"
...
const animatedComicSprites = {
  knight: {
    ...
    walk: [
      ...
      comicSprites.knight,
      loadComicSprite(`assets/sprite-knight-walk-2.png?v=${COMIC_SPRITE_VERSION}`),
      comicSprites.knight,
    ],
    attack: [
      loadComicSprite(`assets/sprite-knight-attack-1.png?v=${COMIC_SPRITE_VERSION}`),
      loadComicSprite(`assets/sprite-knight-attack-2.png?v=${COMIC_SPRITE_VERSION}`),
    ],
...
function drawWorld() {
  ...
  for (let r = 0; r < RAYS; r += 1) {
    ...
    depths[r] = fixedDist;
    const wallH = Math.min(H * 3.25, (H * 2.08) / Math.max(0.001, fixedDist));
    const x = (r / RAYS) * W;
    const colW = W / RAYS + 1;
...
function drawCitadelUpper(townView) {
  ...
  ctx.globalAlpha = townView ? 0.1 : 0.13;
  ...
  ctx.globalAlpha = townView ? 0.1 : 0.16;
  ...
  fog.addColorStop(0.62, "rgba(0, 0, 0, 0.12)");
...
function drawRemoteWarrior(remote, x, y, size) {
  ...
  const edge = "#231812";
  const skin = "#e5b77e";
  const leg = Math.abs(stride) * 1.2;
  drawKnightPlume(x, y, px, false);
  paperRoundedRect(x + 5 * px, y + 2 * px, 14 * px, 10 * px, 3 * px, "#d7d1bd", edge, 0.18);
  paperRoundedRect(x + 6 * px, y + 4 * px, 12 * px, 6.5 * px, 2 * px, skin, edge, 0.16);
  paperRect(x + 5 * px, y + 5 * px, 14 * px, 2 * px, "#484841", edge, 0.08);
  ...
  paperPoly([[x + 18 * px, y + 12 * px], [x + 23 * px, y + 13 * px], [x + 22 * px, y + 20 * px], [x + 18 * px, y + 19 * px]], armor.edge, edge, 0.2);
  drawPaperShield(x + 0.3 * px, y + 13.2 * px, px, armor.light, armor.glint);
  paperRect(x + (6 - leg) * px, y + 21 * px, 5 * px, 5 * px, "#342a24", edge, 0.08);
  paperRect(x + (13 + leg) * px, y + 21 * px, 5 * px, 5 * px, "#342a24", edge, 0.08);
...
function drawPaperKnightSprite(entity, x, y, px) {
  if (!paperKnight.complete || !paperKnight.naturalWidth) return false;
  const width = 28 * px;
  const height = 35 * px;
  ...
  ctx.scale(1 + attack * 0.025, 1 - attack * 0.018);
...
function drawComicSprite(kind, entity, x, y, px, options = {}) {
  const sprite = selectComicSpriteFrame(kind, entity);
...
function selectComicSpriteFrame(kind, entity) {
  const fallback = comicSprites[kind];
  const set = animatedComicSprites[kind];
...
function drawPaperAtlasSprite(kind, entity, x, y, px, options = {}) {
  const cell = PAPER_ATLAS_INDEX[kind];
  if (cell === undefined || !paperAtlas.complete || !paperAtlas.naturalWidth) return false;
  const width = (options.width || 24) * px;
  const height = (options.height || 30) * px;
  const attack = Math.max(0, entity.attackPose || 0);
  const hurt = Math.max(0, entity.hitFlash || 0);
  const windup = Math.max(0, entity.attackWindup || 0);
  const squash = Math.min(0.08, attack * 0.04);
  ctx.save();
  ctx.translate(x + width / 2, y + height / 2);
  ctx.rotate(windup * -0.025 + attack * 0.035);
  ctx.scale(1 + squash, 1 - squash * 0.45);
  if (hurt > 0) ctx.filter = `brightness(${1 + Math.min(0.65, hurt * 1.8)}) saturate(1.15)`;
  ctx.drawImage(
    paperAtlas,
    cell * PAPER_ATLAS_CELL_W,
    0,
    PAPER_ATLAS_CELL_W,
    PAPER_ATLAS_CELL_H,
    -width / 2,
...
function drawRemoteWarriorBack(remote, x, y, px, palette, armor, attack, step) {
  const edge = "#231812";
  const leg = Math.abs(step) * 1.2;
...
function drawRemoteWarriorSide(remote, x, y, px, palette, armor, attack, step, faceLeft) {
  ctx.save();
  if (faceLeft) {
    ctx.translate(x * 2 + 24 * px, 0);
    ctx.scale(-1, 1);
  }
  const edge = "#231812";
  const leg = Math.abs(step) * 1.4;
  drawKnightPlume(x, y, px, false);
  paperPoly([[x + 7 * px, y + 2 * px], [x + 18 * px, y + 3 * px], [x + 20 * px, y + 10 * px], [x + 8 * px, y + 11 * px]], "#d7d1bd", edge, 0.18);
  ...
  paperRect(x + (13 + leg) * px, y + 21 * px, 5 * px, 5 * px, "#342a24", edge, 0.08);
  paperRect(x + (6 - leg) * px, y + 25 * px, 6 * px, 2 * px, "#17110e", edge, 0.04);
  paperRect(x + (13 + leg) * px, y + 25 * px, 6 * px, 2 * px, "#17110e", edge, 0.04);
  drawRemoteSword(x + px * 0.2, y + px * 0.6, px, palette, attack);
  ctx.restore();
}

function drawKnightPlume(x, y, px, back) {
  const edge = "#2a1710";
  paperRect(x + 11 * px, y - 1 * px, 2 * px, 4 * px, "#b9b29f", edge, 0.1);
  paperPoly([
    [x + 12 * px, y - 1 * px],
    [x + (back ? 7 : 8) * px, y - 4.5 * px],
    [x + 5.5 * px, y - 2.2 * px],
    [x + 9.5 * px, y + 0.8 * px],
  ], "#b43a26", edge, 0.16);
  paperPoly([
...
function drawPaperShield(x, y, px, fill, mark) {
  paperPoly([
    [x + 1 * px, y],
    [x + 6 * px, y + 0.8 * px],
    [x + 5.5 * px, y + 7 * px],
  ...
  paperRect(x + 2.7 * px, y + 2 * px, 1.4 * px, 5 * px, mark, "#473421", 0.08);
  paperRect(x + 1.6 * px, y + 3.5 * px, 3.7 * px, 1.3 * px, mark, "#473421", 0.08);
}
...
function spriteScale(e) {
  ...
  if (e.type === "ogre") return 1.14;
  if (e.boss) return 1.42;
  if (e.type === "skeleton") return 0.76;
  if (e.type === "warlock") return 0.84;
  return 0.86;
}
...
function drawPaperSkeletonSprite(e, x, y, px, bone, shade, eye, walk, hurt) {
  ...
  const jawY = hurt ? 11.2 : 10.4;
  paperPoly([[x + 3 * px, y + 3 * px], [x + 6 * px, y + 0.5 * px], [x + 13 * px, y + 0.5 * px], [x + 16 * px, y + 3 * px], [x + 15 * px, y + 11 * px], [x + 12.5 * px, y + 13.5 * px], [x + 6 * px, y + 13.5 * px], [x + 3 * px, y + 10.5 * px]], bone, edge, 0.24);
  paperRect(x + 5 * px, y + 2 * px, 8 * px, 2 * px, "#fff7dc", edge, 0.3);
  paperPoly([[x + 4.5 * px, y + 6 * px], [x + 8.5 * px, y + 6 * px], [x + 8 * px, y + (hurt ? 8 : 10) * px], [x + 5 * px, y + (hurt ? 8 : 10) * px]], eye, edge, 0.08);
  paperPoly([[x + 10 * px, y + 6 * px], [x + 14 * px, y + 6 * px], [x + 13.5 * px, y + (hurt ? 8 : 10) * px], [x + 10.5 * px, y + (hurt ? 8 : 10) * px]], eye, edge, 0.08);
  paperTri(x + 8.1 * px, y + 9 * px, x + 9.2 * px, y + 6.8 * px, x + 10.2 * px, y + 9 * px, shade, edge);
  paperRect(x + 6 * px, y + jawY * px, 7 * px, 2.2 * px, "#3a3026", edge, 0.06);
  paperRect(x + 7 * px, y + jawY * px, 1 * px, 2 * px, "#f4e6c6", edge, 0.05);
  paperRect(x + 9 * px, y + jawY * px, 1 * px, 2 * px, "#f4e6c6", edge, 0.05);
  paperRect(x + 11 * px, y + jawY * px, 1 * px, 2 * px, "#f4e6c6", edge, 0.05);
  paperRect(x + 5 * px, y + 14.5 * px, 9 * px, 2.4 * px, shade, edge, 0.14);
  paperRect(x + 7 * px, y + 16 * px, 5 * px, 6 * px, bone, edge, 0.16);
  paperRect(x + 8.8 * px, y + 16.5 * px, 1.4 * px, 5 * px, "#fff7dc", edge, 0.18);
  paperRect(x + 2.5 * px, y + (14 - walk * 0.8) * px, 3 * px, 8 * px, bone, edge, 0.14);
  paperRect(x + 13.5 * px, y + (14 + walk * 0.8) * px, 3 * px, 8 * px, bone, edge, 0.14);
  drawPaperShield(x - 0.5 * px, y + 14 * px, px * 0.86, "#777a72", "#d2d0bd");
  paperPoly([
    [x + 14.5 * px, y + 14 * px],
    [x + 16 * px, y + 13 * px],
    [x + 20 * px, y + 6 * px],
  ...
  paperRect(x + 14.2 * px, y + 15.5 * px, 4.8 * px, 1.3 * px, "#80613d", edge, 0.1);
  paperRect(x + (5 - step) * px, y + 22 * px, 3.5 * px, 5 * px, bone, edge, 0.14);
  paperRect(x + (10.5 + step) * px, y + 22 * px, 3.5 * px, 5 * px, bone, edge, 0.14);
  paperRect(x + (4 - step) * px, y + 26 * px, 5 * px, 1.8 * px, shade, edge, 0.1);
  paperRect(x + (10 + step) * px, y + 26 * px, 5 * px, 1.8 * px, shade, edge, 0.1);
}

function drawPaperOrcSprite(e, x, y, px, skin, skinLight, shadow, deepShadow, armor, armorLight, eye, walk, attack, winding, hurt) {
  const edge = "#20170f";
  const step = Math.abs(walk) * 1.1;
  const heavy = e.boss || e.type === "ogre" || e.type === "ogreLord";
  const cx = x + 9 * px;
  const headY = y + (heavy ? 7.5 : 7) * px;
  const headW = (heavy ? 8.6 : 7.2) * px;
  const headH = (heavy ? 7.2 : 6.2) * px;

  inkShape(() => {
    ctx.beginPath();
    ctx.moveTo(cx - headW * 0.72, headY - px);
    ctx.lineTo(cx - headW - px * 2.8, headY - px * 2.5);
    ctx.lineTo(cx - headW * 0.88, headY + px * 1.7);
    ctx.lineTo(cx - headW * 0.65, headY + px * 2.3);
    ctx.closePath();
  }, skinLight, edge, Math.max(1.7, px * 0.7));
  inkShape(() => {
    ctx.beginPath();
    ctx.moveTo(cx + headW * 0.72, headY - px);
    ctx.lineTo(cx + headW + px * 2.8, headY - px * 2.5);
  ...
  inkEllipse(cx, headY, headW, headH, skin, edge, Math.max(2, px * 0.85));

  if (heavy) {
    paperTri(cx - 5.5 * px, headY - 4.5 * px, cx - 3.4 * px, headY - 9 * px, cx - 1.2 * px, headY - 4 * px, "#d7c7a2", edge);
    paperTri(cx + 1.2 * px, headY - 4 * px, cx + 3.4 * px, headY - 9 * px, cx + 5.5 * px, headY - 4.5 * px, "#d7c7a2", edge);
  }

  const eyeY = headY - px * 1.2;
  inkLine([[cx - 5 * px, eyeY - (winding ? px : 0)], [cx - 1.5 * px, eyeY]], edge, Math.max(1.3, px * 0.55));
  inkLine([[cx + 1.5 * px, eyeY], [cx + 5 * px, eyeY - (winding ? px : 0)]], edge, Math.max(1.3, px * 0.55));
  inkEllipse(cx - 3.1 * px, eyeY + px * 0.9, px * 1.15, hurt ? px * 0.35 : px * 0.75, eye, "#4a2d0d", Math.max(1, px * 0.4));
  inkEllipse(cx + 3.1 * px, eyeY + px * 0.9, px * 1.15, hurt ? px * 0.35 : px * 0.75, eye, "#4a2d0d", Math.max(1, px * 0.4));
  inkEllipse(cx, headY + px * 1.2, px * 1.2, px * 0.9, deepShadow, edge, Math.max(1, px * 0.38));

  inkShape(() => {
    ctx.beginPath();
    ctx.moveTo(cx - px * 4.6, headY + px * 2.3);
    ctx.quadraticCurveTo(cx, headY + px * 5.4, cx + px * 4.6, headY + px * 2.3);
    ctx.quadraticCurveTo(cx + px * 3.8, headY + px * 5.5, cx, headY + px * 6.2);
    ctx.quadraticCurveTo(cx - px * 3.8, headY + px * 5.5, cx - px * 4.6, headY + px * 2.3);
    ctx.closePath();
  }, shadow, edge, Math.max(1.6, px * 0.65));
  paperTri(cx - 3.8 * px, headY + 3 * px, cx - 2.2 * px, headY + 7 * px, cx - 1.2 * px, headY + 3.7 * px, "#f2e1bc", edge);
  paperTri(cx + 1.2 * px, headY + 3.7 * px, cx + 2.2 * px, headY + 7 * px, cx + 3.8 * px, headY + 3 * px, "#f2e1bc", edge);

  const bodyTop = headY + px * 6;
  inkShape(() => {
    ctx.beginPath();
    ctx.moveTo(cx - (heavy ? 9 : 7) * px, bodyTop + px * 1.5);
    ctx.quadraticCurveTo(cx - 7 * px, bodyTop - px, cx, bodyTop);
    ctx.quadraticCurveTo(cx + 7 * px, bodyTop - px, cx + (heavy ? 9 : 7) * px, bodyTop + px * 1.5);
    ctx.lineTo(cx + (heavy ? 7 : 5.8) * px, bodyTop + px * 10);
    ctx.lineTo(cx - (heavy ? 7 : 5.8) * px, bodyTop + px * 10);
    ctx.closePath();
  }, armor, edge, Math.max(2, px * 0.82));
  inkLine([[cx - 5 * px, bodyTop + px * 2], [cx + 5 * px, bodyTop + px * 2]], armorLight, Math.max(1.2, px * 0.65));
  inkLine([[cx - 4 * px, bodyTop + px * 6], [cx + 4 * px, bodyTop + px * 6]], "#a98242", Math.max(1.4, px * 0.7));
  inkLine([[cx, bodyTop + px * 2], [cx, bodyTop + px * 9]], "#15110d", Math.max(1.1, px * 0.48));

  inkShape(() => {
    ctx.beginPath();
    ctx.ellipse(cx - (heavy ? 8 : 6.5) * px, bodyTop + px * 5, px * 2.7, px * 5.2, -0.18, 0, Math.PI * 2);
  }, skin, edge, Math.max(1.8, px * 0.7));
  inkShape(() => {
    ctx.beginPath();
    ctx.ellipse(cx + (heavy ? 8 : 6.5) * px, bodyTop + px * 5, px * 2.7, px * 5.2, 0.18, 0, Math.PI * 2);
  }, skin, edge, Math.max(1.8, px * 0.7));
  drawPaperShield(cx - 10.5 * px, bodyTop + px * 2.4, px * 1.2, "#666653", "#c4ab5a");

  if (winding || attack > 0) {
    inkLine([[cx + 7 * px, bodyTop + px * 4], [cx + 12 * px, bodyTop], [cx + 14 * px, bodyTop - px * 6]], "#49301b", Math.max(3, px * 1.8));
    inkLine([[cx + 14 * px, bodyTop - px * 6], [cx + 12 * px, bodyTop - px * 9]], "#24170f", Math.max(4, px * 2.2));
  }

  inkShape(() => {
    ctx.beginPath();
    ctx.ellipse(cx - (3.4 + step) * px, bodyTop + px * 12, px * 2.5, px * 4.2, 0.08, 0, Math.PI * 2);
  }, "#29251f", edge, Math.max(1.7, px * 0.68));
  inkShape(() => {
    ctx.beginPath();
    ctx.ellipse(cx + (3.4 + step) * px, bodyTop + px * 12, px * 2.5, px * 4.2, -0.08, 0, Math.PI * 2);
  }, "#29251f", edge, Math.max(1.7, px * 0.68));
...
function drawPaperSkeletonBossSprite(e, x, y, px, bone, shade, eye, walk, hurt, deathKnight) {
  const edge = deathKnight ? "#1a202b" : "#30291e";
  const crown = deathKnight ? "#aeb6c2" : "#ffd467";
  paperRect(x + 2.4 * px, y + 2 * px, 13.2 * px, 10.5 * px, bone, edge, 0.24);
  paperRect(x + 4 * px, y + 1 * px, 10 * px, 2 * px, crown, "#5b3a15", 0.28);
  paperRect(x + 4.8 * px, y + (hurt ? 6 : 7) * px, 3.4 * px, hurt ? 1 * px : 2.4 * px, eye, "#24140f", 0.12);
  paperRect(x + 10 * px, y + (hurt ? 6 : 7) * px, 3.4 * px, hurt ? 1 * px : 2.4 * px, eye, "#24140f", 0.12);
...
function drawPaperWarlockSprite(e, x, y, px, lord, flash, hurt) {
  const edge = lord ? "#21102b" : "#271a32";
  const robe = flash ? "#a982d8" : lord ? "#271032" : "#312044";
  const hood = flash ? "#d7b6ff" : lord ? "#572069" : "#53356c";
  paperTri(x + 1.5 * px, y + 24 * px, x + 8.5 * px, y + 5 * px, x + 16.5 * px, y + 24 * px, robe, edge);
  paperRect(x + 3.2 * px, y + 7.5 * px, 11.6 * px, 16.5 * px, robe, edge, 0.18);
  paperRect(x + 5 * px, y + 14 * px, 8 * px, 2 * px, lord ? "#7a3ba4" : "#6c4b86", edge, 0.16);
  paperPoly([[x + 3 * px, y + 8 * px], [x + 6 * px, y + 1 * px], [x + 12 * px, y + 2 * px], [x + 15 * px, y + 8 * px], [x + 13 * px, y + 11 * px], [x + 5 * px, y + 11 * px]], hood, edge, 0.2);
  paperRoundedRect(x + 5 * px, y + 4.5 * px, 8 * px, 6.5 * px, 2 * px, lord ? "#4c7131" : "#5f8938", edge, 0.14);
  if (lord) {
    paperRect(x + 4 * px, y + 0 * px, 10 * px, 3 * px, "#e2a84a", "#5d3510", 0.25);
  }
  const eye = lord ? "#ffb347" : "#f0d447";
  paperRect(x + 6 * px, y + (hurt ? 7.5 : 6.5) * px, hurt ? 2.4 * px : 2 * px, 1.5 * px, eye, "#19120a", 0.12);
  paperRect(x + 10 * px, y + (hurt ? 7.5 : 6.5) * px, hurt ? 2.4 * px : 2 * px, 1.5 * px, eye, "#19120a", 0.12);
  paperRect(x + 8.4 * px, y + 8 * px, 1.3 * px, 1.5 * px, "#29411d", edge, 0.06);
  paperRect(x + 7 * px, y + 9.5 * px, 5 * px, hurt ? 1 * px : 1.7 * px, "#23120d", edge, 0.08);
  paperTri(x + 7 * px, y + 10 * px, x + 8 * px, y + 12.3 * px, x + 8.6 * px, y + 10 * px, "#efe1c1", "#5e503d");
  paperTri(x + 10.4 * px, y + 10 * px, x + 11 * px, y + 12.3 * px, x + 12 * px, y + 10 * px, "#efe1c1", "#5e503d");
...
function drawPaperBalrogSprite(e, x, y, px, flash, walk, attack, winding) {
  const edge = "#160806";
  const body = flash ? "#ce6a3d" : "#28100d";
  const ember = flash ? "#ffd18b" : "#b52c16";
  const cx = x + 9 * px;
  const wingY = y + 15 * px;

  inkShape(() => {
    ctx.beginPath();
    ctx.moveTo(cx - 3 * px, wingY);
    ctx.quadraticCurveTo(cx - 15 * px, y - 2 * px, cx - 19 * px, y + 6 * px);
    ctx.quadraticCurveTo(cx - 22 * px, y + 15 * px, cx - 15 * px, y + 29 * px);
    ctx.lineTo(cx - 10 * px, y + 20 * px);
    ctx.lineTo(cx - 5 * px, y + 25 * px);
    ctx.closePath();
  }, "#2a0c09", edge, Math.max(2.5, px * 1.1));
  inkShape(() => {
    ctx.beginPath();
    ctx.moveTo(cx + 3 * px, wingY);
    ctx.quadraticCurveTo(cx + 15 * px, y - 2 * px, cx + 19 * px, y + 6 * px);
  ...
  inkLine([[cx - 5 * px, wingY + px], [cx - 15 * px, y + 6 * px], [cx - 12 * px, y + 22 * px]], "#7a2317", Math.max(1.5, px * 0.8));
  inkLine([[cx + 5 * px, wingY + px], [cx + 15 * px, y + 6 * px], [cx + 12 * px, y + 22 * px]], "#7a2317", Math.max(1.5, px * 0.8));

  paperTri(cx - 5 * px, y + 7 * px, cx - 9 * px, y - 3 * px, cx - 1 * px, y + 5 * px, "#5a3821", edge);
  paperTri(cx + 1 * px, y + 5 * px, cx + 9 * px, y - 3 * px, cx + 5 * px, y + 7 * px, "#5a3821", edge);
  inkEllipse(cx, y + 10 * px, 8 * px, 8 * px, body, edge, Math.max(2.4, px));
  inkLine([[cx - 6 * px, y + 7 * px], [cx - 1.5 * px, y + 8.5 * px]], ember, Math.max(1.6, px * 0.75));
  inkLine([[cx + 6 * px, y + 7 * px], [cx + 1.5 * px, y + 8.5 * px]], ember, Math.max(1.6, px * 0.75));
  inkEllipse(cx - 3 * px, y + 10 * px, px * 1.4, px * 0.9, "#ff4b20", edge, Math.max(1, px * 0.45));
  inkEllipse(cx + 3 * px, y + 10 * px, px * 1.4, px * 0.9, "#ff4b20", edge, Math.max(1, px * 0.45));
  inkShape(() => {
    ctx.beginPath();
    ctx.moveTo(cx - 4 * px, y + 14 * px);
    ctx.quadraticCurveTo(cx, y + 17 * px, cx + 4 * px, y + 14 * px);
    ctx.quadraticCurveTo(cx, y + 19 * px, cx - 4 * px, y + 14 * px);
    ctx.closePath();
  }, "#090302", edge, Math.max(1.5, px * 0.65));
  paperTri(cx - 3.5 * px, y + 14.5 * px, cx - 2 * px, y + 18 * px, cx - 1 * px, y + 15 * px, "#e7d4ab", edge);
  paperTri(cx + 1 * px, y + 15 * px, cx + 2 * px, y + 18 * px, cx + 3.5 * px, y + 14.5 * px, "#e7d4ab", edge);

  inkShape(() => {
    ctx.beginPath();
    ctx.moveTo(cx - 9 * px, y + 19 * px);
    ctx.quadraticCurveTo(cx - 7 * px, y + 16 * px, cx, y + 18 * px);
    ctx.quadraticCurveTo(cx + 7 * px, y + 16 * px, cx + 9 * px, y + 19 * px);
    ctx.lineTo(cx + 7 * px, y + 34 * px);
    ctx.lineTo(cx - 7 * px, y + 34 * px);
    ctx.closePath();
  }, "#17100e", edge, Math.max(2.7, px * 1.1));
  for (let i = -2; i <= 2; i += 1) {
    inkLine([[cx + i * 2.2 * px, y + 20 * px], [cx + i * 1.5 * px, y + 32 * px]], i % 2 ? "#5b1a12" : ember, Math.max(1, px * 0.5));
  }
  inkShape(() => {
    ctx.beginPath();
    ctx.ellipse(cx - 9 * px, y + 26 * px, px * 3.4, px * 7, -0.28, 0, Math.PI * 2);
  }, body, edge, Math.max(2.2, px * 0.9));
  inkShape(() => {
    ctx.beginPath();
    ctx.ellipse(cx + 9 * px, y + 26 * px, px * 3.4, px * 7, 0.28, 0, Math.PI * 2);
  }, body, edge, Math.max(2.2, px * 0.9));
  inkEllipse(cx - (4 + Math.abs(walk)) * px, y + 37 * px, px * 3.4, px * 6, "#120806", edge, Math.max(2, px * 0.85));
  inkEllipse(cx + (4 + Math.abs(walk)) * px, y + 37 * px, px * 3.4, px * 6, "#120806", edge, Math.max(2, px * 0.85));

  if (winding || attack > 0) {
    inkLine([[cx + 10 * px, y + 23 * px], [cx + 15 * px, y + 15 * px], [cx + 17 * px, y + 3 * px]], "#ff5a1f", Math.max(3, px * 1.7));
    inkLine([[cx + 17 * px, y + 3 * px], [cx + 18 * px, y - 3 * px]], "#ffd05a", Math.max(1.5, px * 0.75));
  }
}
...
function inkShape(drawPath, fill, edge = "#201710", width = 2.4, highlight = null) {
  ctx.save();
  ctx.lineJoin = "round";
  ctx.lineCap = "round";
  drawPath();
...
function inkEllipse(cx, cy, rx, ry, fill, edge = "#201710", width = 2.4) {
  inkShape(() => {
    ctx.beginPath();
...
function inkLine(points, color = "#201710", width = 2) {
...
function drawHudPanel(x, y, w, h) {
  drawBoardCard(x, y, w, h, { tab: false });
}
...
function drawBoardBar(x, y, w, h, pct, fill, bg, label = "") {
  ...
  ctx.fillStyle = fill;
  ctx.fillRect(Math.round(x + 3), Math.round(y + 3), Math.round((w - 6) * clamped), Math.max(2, Math.round(h - 6)));
  ctx.fillStyle = "rgba(255, 248, 220, 0.34)";
  ctx.fillRect(Math.round(x + 3), Math.round(y + 3), Math.round((w - 6) * clamped), Math.max(2, Math.round((h - 6) * 0.36)));
  if (label) {
    ctx.textAlign = "center";
...
function drawHud() {
  ...
  drawText(`처치 ${kills}`, x2 + 158, bottomCardY + 38, 14, "#4d2b16", { weight: 900 });
  drawText("경험치", x2 + 25, bottomCardY + 86, 13, "#17394a", { weight: 900 });
  drawBoardBar(x2 + 82, bottomCardY + 69, statW - 114, 22, player.xp / player.nextXp, "#5ea9d3", "#101a20", `${compactNumber(player.xp)}/${compactNumber(player.nextXp)}`);

  drawBoardCard(x3, bottomCardY, equipW, bottomCardH, { tab: true });
  drawText(swordName(), x3 + 25, bottomCardY + 38, 16, "#22140c", { weight: 900 });
  drawText(armorName(), x3 + 202, bottomCardY + 38, 16, "#17394a", { weight: 900 });
  ...
  const boss = balrogEnemy() || enemies.find((e) => e.boss && !e.dead);
  if (boss && (Math.hypot(player.x - boss.x, player.y - boss.y) < 8 || boss.hp < boss.maxHp)) {
    drawBoardCard(W / 2 - 180, 22, 360, 44, { dark: true });
    drawText(enemyLabel(boss), W / 2 - 158, 50, 15, "#ffe6a6", { outline: true, weight: 900 });
    drawBoardBar(W / 2 - 36, 34, 182, 16, boss.hp / boss.maxHp, "#b92520", "#260907");
  }
  ...
  if (noticeTimer > 0) {
    ctx.textAlign = "center";
    drawBoardCard(W / 2 - 230, 92, 460, 44, { dark: true });
    drawText(notice, W / 2, 122, 22, "#ffe8a6", { outline: true, weight: 900 });
    ctx.textAlign = "left";
  }
}
